...
def main() -> int:
    ...
    require_markers(".env.example", ["YANDEX_MUSIC_TOKEN=", "STREAMIFY_REPORT_PATH", "STREAMIFY_SNAPSHOT_PATH", "STREAMIFY_RECOMMENDATIONS_DIR", "DBT_THREADS=1"])
    require_markers(
        "dbt/models/yamusic/schema.yml",
    ...
    require_markers("scripts/smoke_dashboard_content.py", ["AppTest", "Streamify Self-Analytics", "Data Quality", "Local data quality signals", "dashboard content exposes the expected self-analytics sections"])
    require_markers("scripts/smoke_compose_local.py", ["docker compose local profile returned HTTP 200", "produced valid local product artifacts", "YANDEX_MUSIC_TOKEN", "wait_for_http", "assert_no_runtime_failures", "run_host_check", "validate_yamusic_raw_contract.py", "smoke_product_answers.py", "smoke_dashboard_content.py", "ModuleNotFoundError"])
